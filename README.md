...
| Day 25 — Open Source vs Closed API Inference | Built an inference benchmark suite comparing local Llama 3.1 8B inference via Ollama against frontier closed models across factual QA, complex PM reasoning, and AI infrastructure systems prompts. Evaluated latency, response quality, deployment economics, and workload-model alignment. Key insight: smaller OSS models were sufficient for constrained retrieval/summarization workloads, while frontier APIs materially outperformed on systems-level reasoning, production infra nuance, and semiconductor architecture tradeoffs. | [Results](https://github.com/souvikkai/souvik-ai-pm-portfolio/tree/main/p2-inference-benchmark) |
